...
VehicleCommand QuadControl::GenerateMotorCommands(float collThrustCmd, V3F momentCmd)
{
  ...
  cmd.desiredThrustsN[0] = (t1 + t2 + t3 + t4) / 4.f;
  cmd.desiredThrustsN[1] = (-t1 + t2 - t3 + t4) / 4.f;
  cmd.desiredThrustsN[2] = (t1 - t2 - t3 + t4) / 4.f;
  cmd.desiredThrustsN[3] = (-t1 - t2 + t3 + t4) / 4.f;
...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
  ...
  if (collThrustCmd > 0) {
	  ...
	  float byp_term = kpBank * by_err;

	  pqrCmd.x = (R(1, 0) * bx_p_term - R(0, 0) * byp_term) / R(2, 2);
	  pqrCmd.y = (R(1, 1) * bx_p_term - R(0, 1) * byp_term) / R(2, 2);
  }
  else {
	  pqrCmd.x = 0.0;
	  pqrCmd.y = 0.0;
